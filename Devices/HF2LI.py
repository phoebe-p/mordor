...
class HF2LI:
    ...
    def open(self, demod=[0,1,2,3]):
        ...
        self.daq.subscribe('/dev1370/demods/'+str(demod[3])+'/sample')

    def measure(self):
        """ Important to know which demodulator the baseline is in, 0 or 3
        (demod 1 or 4)
        """
        self.daq.sync()
        timeout = self.integration_time + 200
        raw_data = self.daq.poll(self.integration_time/1000, timeout)
        Xpump = np.mean(raw_data['dev1370']['demods']['0']['sample']['x'])
        Ypump = np.mean(raw_data['dev1370']['demods']['0']['sample']['y'])
        ...
        Xprobe = np.mean(raw_data['dev1370']['demods']['3']['sample']['x'])
        Yprobe = np.mean(raw_data['dev1370']['demods']['3']['sample']['y'])
        return Xpump, Ypump, Xsum, Ysum, Xdif, Ydif, Xprobe, Yprobe
    ...
    def update_integration_time(self, new_time):
        ...
        Does not work for Zurich lockin
        :return:
        """
    ...
    def close(self, demod=[0,1,2,3]):
        ...
        self.daq.unsubscribe('/dev1370/demods/'+str(demod[3])+'/sample')
