__author__ = 'M. P. Nielsen & A. M. Telford & D. Alonso-Álvarez'
...
class Photoreflectance:
    ...
    def update_header(self):
        ...
        self.header = str(col0)+'\t'+str(col1)+'\t'+str(col2)+'\t'+str(col3)+'\t'+str(col4)+'\t'+str(col5)+'\t'+str(col6)+'\t'+str(col7)+'\t'+str(col8)
        self.header.encode('utf-8')
    ...
    def prepare_scan_lockin(self):
        ...
        self.record = np.zeros((self.size, 9)) ## Data to be saved
    ...
    def mode_lockin(self):
        ...
        if not self.stop:
            Xpump,Ypump,Xsum,Ysum,Xdif,Ydif,Xprobe,Yprobe = self.acquisition.measure()
            ...
            Xdif = Xdif * 2 * sqrt(2)
            Ydif = Ydif * 2 * sqrt(2)
            Xprobe = Xprobe * sqrt(2) * sqrt(2)
            Yprobe = Yprobe * sqrt(2) * sqrt(2)

            self.record[self.i, 1] = Xpump
            self.record[self.i, 2] = Ypump
            self.record[self.i, 3] = Xsum
            self.record[self.i, 4] = Ysum
            self.record[self.i, 5] = Xdif
            self.record[self.i, 6] = Ydif
            self.record[self.i, 7] = Xprobe
            self.record[self.i, 8] = Yprobe

            if self.plot_format['Ch1_ylabel'] == 'Xsignal/Rbaseline':
                self.plotdata[self.i, 1] = Xsum / sqrt(Xprobe**2+Yprobe**2)
            elif self.plot_format['Ch1_ylabel'] == 'Rsignal/Rbaseline':
                self.plotdata[self.i, 1] = sqrt(Xsum**2+Ysum**2) / sqrt(Xprobe**2+Yprobe**2)
            ...
            if self.plot_format['Ch2_ylabel'] == 'Rbaseline':
                self.plotdata[self.i, 2] = sqrt(Xprobe**2+Yprobe**2)
            else:
    ...
    def run_check(self):
        ...
        self.run_ok = []
        self.check_inputs('Start wavelength', self.Start_wave, 250, 3000)
        self.check_inputs('End wavelength', self.Stop_wave, 250, 3000)
        self.check_inputs('Step', self.Step_wave, 0.1, 3000)
    ...
    def live_lockin(self):
        ...
        if not self.stop:
            ...
            Xdif = Xdif * 2 * sqrt(2)
            Ydif = Ydif * 2 * sqrt(2)
            Xprobe = Xprobe * sqrt(2)
            Yprobe = Yprobe * sqrt(2)

            if self.plot_format['Ch1_ylabel'] == 'Xsignal/Rbaseline':
                self.live_data[-1, 1] = Xsum / sqrt(Xprobe**2+Yprobe**2)
            elif self.plot_format['Ch1_ylabel'] == 'Rsignal/Rbaseline':
                self.live_data[-1, 1] = sqrt(Xsum**2+Ysum**2) / sqrt(Xprobe**2+Yprobe**2)
            elif self.plot_format['Ch1_ylabel'] == 'Xsignal':
            ...
            if self.plot_format['Ch2_ylabel'] == 'Rbaseline':
                self.live_data[-1, 2] = sqrt(Xprobe**2+Yprobe**2)
            else:
